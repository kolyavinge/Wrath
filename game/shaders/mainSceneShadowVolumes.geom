...
{
    vec3 lightPositionView = vec3(modelViewMatrix * vec4(light, 1.0));
...
void emitEdgeQuad(vec3 light, vec3 a, vec3 b)
{
    ...
    gl_Position = projectionMatrix * vec4(a - lightPositionView, 0.0);
    EmitVertex();

    gl_Position = projectionMatrix * vec4(b, 1.0);
    EmitVertex();

    gl_Position = projectionMatrix * vec4(b - lightPositionView, 0.0);
    EmitVertex();
