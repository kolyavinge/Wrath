...
void emitEdgeQuad(vec3 lightPositionView, vec3 a, vec3 b)
{
    ...
    gl_Position = projectionMatrix * vec4(a + deviation, 1);
    EmitVertex();

    gl_Position = projectionMatrix * vec4(lightDirection, 0);
    EmitVertex();

    lightDirection = normalize(b - lightPositionView);
    deviation = lightDirection * epsilon;
    gl_Position = projectionMatrix * vec4(b + deviation, 1);
    EmitVertex();
...
void processLight(int lightIndex)
{
    ...
    if (facesLight(lightPositionView, PositionView[0], PositionView[2], PositionView[4]))
    {
        ...
        vec3 deviation = lightDirection * epsilon;
        gl_Position = projectionMatrix * vec4(PositionView[0] + deviation, 1);
        EmitVertex();

        lightDirection = normalize(PositionView[2] - lightPositionView);
        deviation = lightDirection * epsilon;
        gl_Position = projectionMatrix * vec4(PositionView[2] + deviation, 1);
        EmitVertex();

        lightDirection = normalize(PositionView[4] - lightPositionView);
        deviation = lightDirection * epsilon;
        gl_Position = projectionMatrix * vec4(PositionView[4] + deviation, 1);
        EmitVertex();
        EndPrimitive();
