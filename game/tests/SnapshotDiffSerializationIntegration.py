...
class SnapshotDiffSerializationIntegration(unittest.TestCase):
    ...
    def test(self):
        diff = SnapshotDiff()
        diff.addedEnemyIds = [1, 2, 3, 4, 5]
        diff.removedEnemyIds = [10, 20, 30, 40]
        diff.person = SnapshotPersonDiff.makeFull(1, Vector3(float32(3.2), float32(-6.9), 4345434.87532), 1.5, -1.5, 10, 1.0, 2)
        ...
        diff.enemies = [
            SnapshotPersonDiff.makeFull(10, Vector3(float32(3.2), float32(-6.9), float32(44.2)), 2.5, -2.5, 50, 0.5, 1),
            SnapshotPersonDiff.makeFull(20, Vector3(float32(3.2), float32(-6.9), float32(44.2)), 2.0, -2.0, 60, 0.5, 4),
        ]
        diff.addedBullets = [
            SnapshotBullet.make(
        ...
        self.assertEqual(diffDeser.addedEnemyIds, diff.addedEnemyIds)
        self.assertEqual(diffDeser.removedEnemyIds, diff.removedEnemyIds)
